...
public static class FileUtil
{
    ...
    public static int GetBufferSize(long size) => 
        size switch
        {
            <= 0 or > StorageConfig.MaxFileSize => throw new Exception("Impossible"),
            <= MaxSmallFileSize => StorageConfig.SmallBufferSize,
            <= MaxMediumFileSize => StorageConfig.MediumBufferSize,
            _ => StorageConfig.LargeBufferSize
        };
}