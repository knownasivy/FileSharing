...
public class MetadataService : IMetadataService
{
    ...
    public async Task<bool> HandlePreviewFileAsync(UploadFile file)
    {
        ...
        {
            _logger.LogError(ex, "Failed to handle preview file for {FileId}", file.Id);
            return false;
        }
        finally
        {
    ...
    private static void ExtractAttachedPicture(IMediaAnalysis mediaInfo, AudioMetadata metadata)
    {
        ...
        if (videoStream?.Disposition != null &&
    ...
    private static void ExtractTagMetadata(IMediaAnalysis mediaInfo, AudioMetadata metadata)
    {
        ...
        var filteredTags = mediaInfo.Format.Tags
            .AsValueEnumerable()
    ...
    private async Task<string?> DetermineUploadFileAsync(UploadFile file, string localPreview, List<string> tempFiles)
    {
        var hasLocalPreview = file.Extension is "m4a" or "mp3" && File.Exists(localPreview);
        
        _logger.LogInformation("Has local preview: {Value}", hasLocalPreview);
    ...
    private async Task UploadPreviewFileAsync(string uploadFile, string previewFileName, Guid fileId)
    {
        ...
        await _cloudService.UploadAsync(previewFileName, uploadFile, "video/mp4");
        await _cache.RemoveAsync($"UploadFileService:GetDownloadFileByIdAsync:{fileId}");
        await _cache.RemoveAsync($"preview:{previewFileName}");
    }
    ...
    private async Task HandleCoverArtAsync(UploadFile file)
    {
        ...
        {
            _logger.LogError(ex, "Failed to upload cover art for file {FileId}", file.Id);
        }
        finally
        {
    ...
    private async Task DeleteFileWithLoggingAsync(string filePath)
    {
        ...
        {
            _logger.LogWarning(ex, "Access denied when trying to delete file: {FilePath}", filePath);
        }
        catch (IOException ex)
        {
            _logger.LogWarning(ex, "I/O error when trying to delete file: {FilePath}", filePath);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error when trying to delete file: {FilePath}", filePath);
        }
    }

    public static async Task<string?> ConvertAudioToPreviewFileFastAsync(UploadFile file)
    {
        var outputFile = Path.GetFullPath(Path.Combine(file.GetLocation(), $"{file.Id:N}_prev.mp4"));
        var coverVideoPath = Environment.GetEnvironmentVariable("COVER_VIDEO_PATH") 
                             ?? "cover.mp4";
    ...
    private static async Task<string?> ExtractAudioCoverArt(UploadFile file)
    {
        ...
        List<string> args = [
            ...
            "-map_metadata", "-1",
            "-an", 
            "-vf", "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2,format=yuv420p",
            "-c:v", "libwebp",
            "-lossless", "0",
            "-compression_level", "3",
            "-qscale:v", "80",
